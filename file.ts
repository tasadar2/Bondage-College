export class File {
    ...
    public static openFile(callback: (content: string) => void) {
        ...
        input.type = "file";
        input.click();
        input.onchange = (e) => File.readInput(e.target as HTMLInputElement, callback);
    }
    ...
    private static readInput(input: HTMLInputElement, callback: (content: string) => void) {
        if (input && input.files && input.files.length === 1) {
            ...
            reader.onload = (readerEvent) => File.readContent(readerEvent.target as FileReader, callback);
        }
    }
